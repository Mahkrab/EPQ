...
[mathematics explanation](explation.md) explains the same mathematics in simple language but does not
...
Milestone 00 uses only static plane projection. A plane has a finite unit normal $\mathbf{n}$ pointing into the permitted half-space and a finite offset $d$ in metres:
